...
const generate = (count, chunk) => {
  for (let j = 0; j < chunk; j++) {
    ...
    tripWriter.pipe(fs.createWriteStream(`db/trip${j}.csv`));
    ...
    packageWriter.pipe(fs.createWriteStream(`db/package${j}.csv`));
...
const postgresChunker = (count, file) => {
  ...
  const tripWriter = csvWriter();
  tripWriter.pipe(fs.createWriteStream(`db/pgtrip${file}.csv`));
  for (let trip of trips) {
    tripWriter.write(trip);
  }
  tripWriter.end();
  ...
  const packageWriter = csvWriter();
  packageWriter.pipe(fs.createWriteStream(`db/pgpackage${file}.csv`));
  for (let package of packages) {
    packageWriter.write(package);
  }
  packageWriter.end();
